...
struct StatsCurrencyNotice: View {
    var body: some View {
        HStack(spacing: 8) {
            Image(systemName: "info.circle")
                .foregroundStyle(Color.accentColor)
            Text("统计图表统一按设置汇率折算为 HKD，交易卡片仍保留原币种提示。")
                .font(.caption)
                .foregroundStyle(.secondary)
            Spacer(minLength: 0)
        }
...
struct TagPerformanceSection: View {
    let title: String
...
struct SymbolTradeListView: View {
    let ticker: String
